...
//Code for expiration date verification checks..
    ...
    //     // Add a new product with an expiration date
    //     function addProduct(bytes32 _name, uint256 _expirationDate) public {
    //         productCount++;
    //         products[productCount] = Product(productCount, _name, _expirationDate);
    //     }

    //     // Check if a product is expired
    //     function isProductExpired(uint256 _productId) public view returns (bool) {
    //         require(_productId <= productCount, "Invalid product ID");

    //         Product memory product = products[_productId];
    //         return (block.timestamp > product.expirationDate);
    //     }
    // }
// we can also use timestamps to confirm using block.timestamp function call